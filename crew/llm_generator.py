...
RESPONDA SEMPRE EM JSON ESTRUTURADO."""
    ...
    @staticmethod
    def _normalize_language_request(lang: Optional[str]) -> Optional[str]:
        """Normaliza o idioma solicitado para nomes canônicos em inglês."""
        if not lang:
            return None
        raw = str(lang).strip()
        if not raw:
        ...
        # Remove possíveis emojis/bandeiras no início
        raw = re.sub("^[\U0001F1E6-\U0001F1FF]{2}\\s*", "", raw)
        key = raw.lower()
    ...
    async def generate(
        ...
        try:
            ...
            # Detecta/normaliza idioma solicitado (ou mantém pt-BR como padrão)
            if language_request:
                language_request = self._normalize_language_request(language_request)
            if not language_request:
                language_request = self._normalize_language_request(
                    self._detect_language_request(query)
                )
